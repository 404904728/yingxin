							...
							if (e[0] == "*") {
								...
								for ( var m = 0; m < e.length; m++) {
									...
									if($("#attach"+e[m])){
										//删除附件
										$.ajax( {
											url :hmq.path("attach.do?delete"),
											dataType : "json",
											data : {"id":$("#attach"+e[m]).val()},
											async : false,
											cache : false,
											type : "post"
										});
		...
		onSelect : function(h) {
			...
			for ( var g in this.queueData.files) {
				...
				if (f.uploaded != true && f.name == h.name) {
					...
					**/
					var e=confirm("您上传的文件("+h.name+")已经存在,是否覆盖");
					if (!e) {
			...
			if (c.inArray("onSelectError", e.overrideEvents) < 0) {
				...
				case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
					if (e.queueSizeLimit > f) {
						...
						this.queueData.errorMsg+="所选文件的数量超过剩余的上传限制";
					} else {
						//this.queueData.errorMsg += "\nThe number of files selected exceeds the queue size limit ("
						//		+ e.queueSizeLimit + ").";
						this.queueData.errorMsg+="所选文件的数量超过了队列大小限制("+e.queueSizeLimit+")";
					}
					break;
				case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
					...
					//		+ ").";
					this.queueData.errorMsg+="文件("+d.name+")超过大小限制("+e.fileSizeLimit+")"
					break;
				case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
					//this.queueData.errorMsg += '\nThe file "' + d.name + '" is empty.';
					this.queueData.errorMsg += "文件("+d.name+")是空的,不能上传";
					break;
				case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
					//this.queueData.errorMsg += '\nThe file "' + d.name
					//		+ '" is not an accepted file type ('
					//		+ e.fileTypeDesc + ").";
					this.queueData.errorMsg += "文件("+d.name+")是不能被允许的类型";
					break;
				}
			}
